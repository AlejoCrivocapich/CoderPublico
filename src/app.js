...
const fs = require('fs')
const ProductManager = require('./manager/manager.js'); // importar archivo que contine la clase "ProductManager"
...
const port = 8080; 
const pm = new ProductManager('routes/products.json'); // creacion de instancia

let carts = [];//array carts vacio
...
app.get('/home', (req, res) => {
  try {
      ...
      const productsData = fs.readFileSync('routes/products.json', 'utf-8');
...
cartsRouter.post('/', (req, res) => { //endpoint de carts para ingresar un nuevo cart mediante post
  ...
  fs.writeFileSync('routes/carts.json', JSON.stringify(carts)); // modificamos el archivo con los cambios
...
cartsRouter.post('/:cid/product/:pid', (req, res) => { // endpoint de carts para ingresar en products de carts la id del producto con la misma
  ...
  fs.writeFileSync('routes/carts.json', JSON.stringify(carts));//modifcamos el archivo
  res.writeHead(202, { "content-type": "text/html; charset=utf8" });
...
io.on('connection', (socket) => {
    ...
    // Escuchar eventos para productos nuevos o eliminados
